...
const CompanyEmployees = () => {
    ...
    const confirmDelete = async () => {
        ...
        await deleteCompanyUser(selectedUser.id).unwrap();
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" aria-modal="true" role="dialog">
            <div className="bg-surface p-6 sm:p-8 rounded-2xl shadow-lg w-full max-w-sm m-4 animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="flex justify-end gap-4">
                    <button onClick={() => setIsDeleteConfirmOpen(false)} className="px-6 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 font-semibold transition-colors">
                        {t('cancel')}
                    </button>
                    <button onClick={confirmDelete} disabled={isDeleting} className="px-6 py-2 rounded-lg bg-red-500 text-white hover:bg-red-600 font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-50">
        ...
        <div className="p-4 sm:p-8 animate-in fade-in duration-300">
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
                 <div className="order-2 md:order-1">
                    <h1 className="text-2xl font-bold">{t('company_employees', 'Company Employees')}</h1>
                    <p className="text-text-muted">{t('manage_company_users_roles_desc', 'Manage user accounts and roles for company staff.')}</p>
                </div>
                <div className="order-1 md:order-2 flex justify-start md:justify-end">
                    <Link href="/company-user/add" className="px-6 py-3 bg-brand-primary text-white font-bold rounded-xl flex items-center justify-center gap-2 hover:bg-brand-primary/90 transition-all shadow-lg shadow-brand-primary/20 cursor-pointer w-full sm:w-auto">
                        <UserPlus size={20} />
                        {t('add_user')}
                    </Link>
                </div>
            ...
            {error ? (
                ...
                <EmployeeTable users={users} handleDeleteUser={handleDeleteUser} editPathPrefix="/company-user/edit" isLoading={isLoading || isFetching} />
            )}
